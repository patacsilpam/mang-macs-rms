    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="mt-3">
          ...
          <input type="text" class="form-control border border-danger" id="inputProductPrice" value="<?=$total_price; ?>" oninput="calculateNoDiscount()" readonly>
        </div>
        <div class="mt-3">
          <label>Amount of Pay</label>
          <input type="number" class="form-control" id="inputPaid" name="amountPay" oninput="calculateNoDiscount()" onkeyup="EnableDisable(this)" required>
        </div>
        <div class="mt-3">
          <strong>Dicount(<i>For Senior Citizen and PWD only</i>)</strong>
          <label>Customer Type</label>
          <select class="form-control" id="selectedCustomer" name="selectedCustomer" onchange="chooseDiscount()" disabled>
        ...
        <div id="discount" style="display:none;" class="mt-3">
          <div class="mt-3">
            ...
            <div style="display: flex; flex-direction:row">
              <input type="text" class="form-control" id="inputDiscount" name="discount" value="20" readonly oninput="calculateNoDiscount()">
              <span>&nbsp;%</span>
            </div>
          </div>
          <div class="mt-3">
            <label>ID No.</label>
            <input type="text" class="form-control" name="idNumber" placeholder="Id No.">
          </div>
          <div class="mt-3">
            <label>Discounted Price:</label>
            <input type="number" class="form-control border border-primary" name="discountedPrice[]" id="totalPrice" readonly>
          </div>
          <hr>
        </div>
        <div class="mt-3">
          <label> Change: </label>
          <input class="form-control border border-success" name="returnChange" id="change" readonly>
        </div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
        <button type="submit" class="btn btn-success" name="btn-save-cart">Enter</button>
      </div>
    </div>
  </div>
</div>
